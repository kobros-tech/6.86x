...
where $\tilde X$ stacks the centered examples as rows (this is the same $X^TX$ structure that appeared in the closed-form linear regression solution in Lecture 5, but used here for a different purpose). $S$ is proportional to the covariance matrix of the data.
...
1. PCA finds orthogonal directions (principal components) of maximum variance in the data by taking the eigenvectors of the centered scatter matrix $X^TX$, sorted by eigenvalue.
