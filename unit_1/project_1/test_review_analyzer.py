...
class ReviewAnalyzerTests(unittest.TestCase):
    ...
    def test_pegasos_batch_update_uses_full_batch_denominator(self) -> None:
        # With a two-example batch, only one example violates the margin at
        # the first step. The hinge update must still be scaled by batch_size.
        # A one-feature positive example gives an especially transparent case.
        data = [(1, {0: 1.0}), (1, {})]
